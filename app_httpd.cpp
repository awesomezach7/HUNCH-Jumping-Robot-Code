...
#include <s3servo.h>                                  //https://github.com/SimGallery/ESP32S3servo
const int BLDC1_pin = 44; //last
const int BLDC2_pin = 7; //2nd to bottom
const int SERVO1_pin = 8; //last
const int SERVO2_pin = 9; //2nd to bottom
s3servo BLDC1;
s3servo BLDC2;
s3servo SERVO1;
s3servo SERVO2;
double bldc1_power = 0;
double bldc2_power = 0;
double servo1_power = 0;
double servo2_power = 0;
...
static esp_err_t cmd_handler(httpd_req_t *req)
{
    ...
    else if (!strcmp(variable, "pwm1")){
        bldc1_power = val;
        BLDC1.write((int) bldc1_power);
        Serial.println(bldc1_power);
    }
    else if (!strcmp(variable, "pwm2")){
        bldc2_power = val;
        BLDC2.write((int) bldc2_power);
        Serial.println(bldc2_power);
    }
    else if (!strcmp(variable, "pwm3")){
        servo2_power = val;
        SERVO1.write((int) servo1_power);
        Serial.println(servo1_power);
    }
    else if (!strcmp(variable, "pwm4")){
        servo2_power = val;
        SERVO2.write((int) servo2_power);
        Serial.println(servo2_power);
    }
...
static esp_err_t status_handler(httpd_req_t *req)
{
    ...
    *p++ = '{';
    ...
    p += sprintf(p, "\"pwm1\":%u,", bldc1_power);
    p += sprintf(p, "\"pwm2\":%u,", bldc2_power);
    p += sprintf(p, "\"pwm3\":%u,", servo1_power);
    p += sprintf(p, "\"pwm4\":%u,", servo2_power);
...
static esp_err_t index_handler(httpd_req_t *req)
{
    ...
    if (s != NULL) {
        if (s->id.PID == OV3660_PID) {
            return httpd_resp_send(req, (const char *)new2_html_gz, new2_html_gz_len);
        }
    } else {
